...
export default function App() {
  ...
  return (
    <div style={{ width:'100%', maxWidth: '900px', margin: '0 auto', padding: '40 20px', display: 'flex',flexDirection:'column', gap: '40px', fontFamily: 'system-ui, sans-serif' }}>
      <header style={{ textAlign: 'center', marginBottom: '40px' }}>
        <img src={logo} alt="Dragon Paper Vault Logo" style={{ height: '80px', marginBottom: '12px' }} />
        <h1 style={{ color: '--text-h', margin: '0 0 8px 0' }}>Dragon Paper Vault</h1>
        <p>Let us all impart our knowledge with everyone.</p>
      </header>

      {/* FILTER PANEL */}
      <section style={{ background: 'var(--code-bg)', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px', marginBottom: '40px' }}>
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--text-h)' }}>🔍 Search Filters</h3>
      ...
      {/* 2. RESULT SHOWCASE */}
        <section style={{ marginBottom: '40px', textAlign:'center' }}>
          ...
          {!selYear ? (
            <p style={{ color: 'var(--text-h)', textAlign: 'center', padding: '20px' }}>Please configure all dropdown filters above to locate files.</p>
          ) : papers.length === 0 ? (
            <p style={{ color: 'var(--text-h)' }}>No documents stored inside this specific folder path yet.</p>
          ) : (
            <div style={{ display: 'grid', gap: '12px' }}>
              {papers.map((paper, idx) => (
                <div key={idx} style={{ padding: '16px', border: '1px solid var(--border)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#ffffff' }}>
                  <span style={{ fontWeight: '500', flexWrap:'wrap', gap:'12px'}}>{paper.name}</span>
                  <a 
      ...
      {/* UPLOAD FORM */}
      <section style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '24px', background: 'var(--bg)', textAlign: 'center' }}>
        <h3 style={{ margin: '0 0 16px 0', color: 'var(--text-h)'}}>📤 Contribute a Paper</h3>
        <form onSubmit={handleUpload} className="upload-form-grid" style={{ justifyContent: 'space-between' }}>
          <input type="text" placeholder="Subject (e.g., Physics)" value={upSub} onChange={e => setUpSub(e.target.value)} style={{ padding: '10px', borderRadius: '6px', margin:'7px 7px 7px 7px' }} required />
          <input type="text" placeholder="School (e.g., Lincoln Higher Secondary School)" value={upSchool} onChange={e => setUpSchool(e.target.value)} style={{ padding: '10px', borderRadius: '6px', margin:'7px 7px 7px 7px' }} required />
          <input type="text" placeholder="Class (e.g., 11)" value={upClass} onChange={e => setUpClass(e.target.value)} style={{ padding: '10px', borderRadius: '6px',margin:'7px 7px 7px 7px' }} required />
          <input type="number" placeholder="Year (e.g., 2026)" value={upYear} onChange={e => setUpYear(e.target.value)} style={{ padding: '10px', borderRadius: '6px',margin:'7px 7px 7px 7px' }} required />
          <input type="file" onChange={e => setFile(e.target.files[0])} style={{ gridColumn: '1 / -1' }} required />
          <button type="submit" style={{ gridColumn: '1 / -1', padding: '12px', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>Submit</button>
        </form>
      ...
      <footer style={{ padding: '40px 0 0 0 ', borderTop: '4px solid #3b82f6', fontSize: '14px', textAlign: 'left', margin: '20px 20px 20px 20px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '30px' }}>
          ...
          <div >
            <h4 style={{ color: 'var(--text-h)', margin: '0 0 12px 0', fontSize: '16px' }}>🐉 Dragon Paper Vault</h4>
          ...
          <div style={{ flex: '1 1 200px' }}>
            <h4 style={{  color: 'var(--text-h)', margin: '0 0 12px 0', fontSize: '16px' }}>Contact</h4>
            <p style={{ margin: '0 0 6px 0' }}>📬 Email: <span style={{ color: 'var(--text-h)' }}>vivekbennysharma@gmail.com</span></p>
            <p style={{ margin: 0 }}>🛠️ Founded and Maintained by <b><a href="https://www.facebook.com/vivek.benny.sharma">Vivek Benny Sharma</a></b></p>
          </div>
